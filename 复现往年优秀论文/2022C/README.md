...
需要Python 3.11或更新版本。以下命令从仓库根目录进入 `复现往年优秀论文/2022C` 并安装依赖：
...
默认附件路径由 `scripts_tables/constants.py` 按文件位置定位，相对于 `2022C` 目录为：
...
解题入口和绘图脚本统一从 `scripts_tables` 包导入计算模块。入口会根据自身文件位置定位 `2022C`，可从其他工作目录通过脚本路径启动；默认附件和默认输出目录也按项目位置定位。

导入修复后，8个解题入口和91个绘图入口已分别从项目外的临时目录执行成功，生成的41个CSV和91张PNG与已有结果逐文件一致。验证输出写入临时目录，原有 `outputs/` 文件保持不变。

在 `2022C` 目录下，可以直接运行以下命令：
...
| `--output` | `2022C/outputs` | 指定结果根目录；表和图分别写入其 `tables/`、`figures/` |
