"""附录B 铅钡第2分支二次聚类。来源：PDF p33-34。附录代码会生成的图，正文未单列；补写。"""
import sys
...
# 绘制并保存铅钡样本初次二分后第2分支的Ward聚类树
def draw(data, args):
    frame = data.samples.loc[data.samples['类型'].eq('铅钡')]
    x = data.matrix(frame)
    outer = ward_labels(x)
    frame = frame.iloc[np.flatnonzero(outer == 1)]
    ...
    ax.set_title('铅钡：二类聚类后第2分支再次聚类\n分支编号由算法生成，没有时间含义')
    finish(fig, args, 'appendixB_lead_barium_branch2')
