"""图6c 高钾风化相关矩阵。来源：PDF p21。按正文补写；非零子组成log-ratio；常量相关遮罩。"""
import sys
...
# 绘制并保存高钾风化样本的成分对数比相关矩阵热图
def draw(data, args):
    frame = subgroup(data, '高钾', '风化')
    ...
    ax.set_title('高钾风化：log-ratio Pearson相关\n灰色为恒定列，相关系数未定义')
    fig.colorbar(image, ax=ax, shrink=.78, label='Pearson r（不是因果关系）')
    finish(fig, args, 'fig06c_correlation')
