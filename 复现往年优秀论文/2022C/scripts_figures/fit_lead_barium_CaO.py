"""附录B 铅钡CaO回归拟合。来源：PDF p35循环；数值对应p14-15表13-16。附录代码会生成但PDF未单列的拟合图；重算。"""
import sys
...
# 绘制并保存铅钡玻璃CaO的四阶段中心与二次回归曲线
def draw(data, args):
    fits, _ = fit_trends(data)
    r = fits.loc[fits['类型'].eq('铅钡') & fits['成分'].eq('CaO')].iloc[0]
