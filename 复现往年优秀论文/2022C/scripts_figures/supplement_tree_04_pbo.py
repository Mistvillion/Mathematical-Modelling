"""补充 无风化按原文预选PbO重训树。来源：对应PDF p18-20。新增训练结果，非论文原树。"""
import sys
...
# 绘制并保存高钾玻璃无风化组的PbO重训决策树
def draw(data, args):
    model = fitted_pbo_models(data, args.seed)['无风化']
    ...
    ax.set_title('无风化：按原文预选PbO重训树（全部有效样本）')
    finish(fig, args, 'supplement_tree_04_pbo')
