"""补充 风化14成分自由选择重训树。来源：对应PDF p18-20。新增训练结果，非论文原树。"""
import sys
...
# 绘制并保存高钾玻璃风化组的14成分重训决策树
def draw(data, args):
    model = fitted_models(data, args.seed)['风化']
    ...
    ax.set_title('风化：14成分自由选择重训树（全部有效样本）')
    finish(fig, args, 'supplement_tree_05_all')
