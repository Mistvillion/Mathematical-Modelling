"""附录A 高钾原始与预测混合聚类。来源：PDF p30-31（高钾图10）。平均联接重算；星号为本包预测；仅相似性对照。"""
import sys
...
# 绘制并保存高钾原始样本与预测样本的混合聚类树
def draw(data, args):
    z, membership = mixed_cluster_check(data, '高钾')
    ...
    ax.set_title('高钾：原始与预测数据的混合聚类\n* 为修正版预测；不是预测准确性的独立验证')
    finish(fig, args, 'appendixA_potassium_mixed_cluster')
