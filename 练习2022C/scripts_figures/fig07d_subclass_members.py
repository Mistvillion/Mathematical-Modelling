"""图7d 铅钡风化亚类成员。来源：PDF p23。按表25特征重算的成员图；不冒充原图逐成员结果。"""
import sys
...
def draw(data, args):
    ...
    ax.set_ylim(0, 1)
    ax.axis('off')
    ax.text(.5, .88, '铅钡风化', ha='center', bbox={'boxstyle':'round', 'fc':'#b8d7e8', 'ec':'#315b78'}, fontsize=14)
    ...
    ax.set_title('亚类划分：铅钡风化\n特征：' + ', '.join(features) + '；原始含量，Ward，k=2')
    finish(fig, args, 'fig07d_subclass_members')
